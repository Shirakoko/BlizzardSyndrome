...
=======
public class NPC_Adam : BaseNPC
{
    public Animator moveAnim; // 用来控制人物走动的动画控制器，挂在NPC2上
    public int movedCount; // 已经运动过的次数
>>>>>>> 90c4bc3 (1208)
...
<<<<<<< HEAD
        if(isMoved==true && (Time.frameCount % 5 == 0))
        {
            ...
            if(transform.position==new Vector3(13,-5.9f,38)&&talkCount==1)
            {
                isMoved = false;
=======
        if(movedCount>0 && (Time.frameCount % 5 == 0))
        {
            // 如果NPC到达终点，要停下走路动画
            if(transform.position==new Vector3(13,-5.9f,38)&&talkCount==1&&movedCount==1)
            {
                movedCount++;
>>>>>>> 90c4bc3 (1208)
...
=======
            else if(transform.position==new Vector3(14,-5.9f,80)&&movedCount==3) // 到达第二个位置后消失
>>>>>>> 90c4bc3 (1208)
            {
...
=======
    ...
    {
        if(movedCount==0 && !isTalked) // 若还在说话阶段
>>>>>>> 90c4bc3 (1208)
        {
            ShowPrompt();
...
=======
        if(isTalked == true)
        {
            if(talkCount==1 && movedCount==0)
            {
                moveAnim.SetTrigger("Move");
                animController.Play("jump");
                movedCount++;
            }
            else if(talkCount==2 && movedCount==2)
            {
                moveAnim.SetTrigger("Move2");
                animController.Play("jump");
                movedCount++;
